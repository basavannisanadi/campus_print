      ...
      sse.onmessage = (event) => {
        try {
          ...
          if (data.type === 'connected') {
            ...
            fetchJobs();
          } else if (data.type === 'printer_updated') {
            setPrinterStatus(data.settings.status);
            setExpectedReturnTime(data.settings.expectedReturnTime);
            setAveragePrintSpeed(data.settings.averagePrintSpeed);
          } else if (data.type === 'shop_updated') {
            // Keep printer status synced
            if (data.shop && data.shop.id === 'alliance_print') {
              setPrinterStatus(data.shop.printerStatus);
            }
          }
        } catch (err) {
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-slate-500 hidden sm:block mt-0.5">
                  Fast, reliable campus printing (Testing Mode)
                </p>
              </div>
            </div>

            {/* Printer Status & Admin Toggle */}
            <div className="flex items-center gap-3">
              {currentRoute === '/admin' && (
                <button
                  onClick={() => navigate('/')}
